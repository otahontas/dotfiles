...
local function highlight_overdue()
  ...
  for lnum, line in ipairs(lines) do
    local col_start, col_end, due_date = line:find("(due:%d%d%d%d%-%d%d%-%d%d)")
    if due_date then
      local date = due_date:match("due:(%d%d%d%d%-%d%d%-%d%d)")
      if date and date <= today then
        table.insert(positions, {lnum, col_start, col_end - col_start + 1})
      end
    end
  end

  if #positions > 0 then
    match_id = vim.fn.matchaddpos("ErrorMsg", positions, 10)
  end
end
